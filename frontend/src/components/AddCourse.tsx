    ...
    function addLessonRow() {
        const newLesson = (
            <div key={rowNumber} id={rowNumber.toString()} ref={lessonContainer} className='bg-white border-[1px] border-gray-500 rounded-lg p-3 relative flex flex-wrap gap-2 my-2 pt-6 lg:gap-10 xl:w-[80%]'>
                <input {...register('lessonTitle')} className='py-1 pl-2 bg-[#01c8b5] outline-hidden placeholder-black rounded-lg lg:w-60' type='text' placeholder='Lesson Title' />
                <input {...register('videoFile')} className='border-2 border-gray-300 w-44 lg:w-60' type='file' placeholder='Lesson Video' />
                <input {...register('duration')} className='py-1 pl-2 bg-[#01c8b5] outline-hidden placeholder-black rounded-lg lg:w-60' type='text' placeholder='Lesson Duration' />
  ...
  return (
    <form onSubmit={handleSubmit(addCourse)} encType='multipart/form-data'>
        <div className='w-full flex flex-col gap-12 px-10 py-14 xl:px-36'>
            <input {...register('title')} className='w-60 h-10 bg-[#01c8b5] outline-hidden text-black placeholder-black text-base font-medium p-4 rounded-lg lg:w-80 lg:text-lg xl:w-96' type="text" placeholder='Course Title'/>
            <input {...register('description')} className='w-60 h-10 bg-[#01c8b5] outline-hidden text-black placeholder-black text-base font-medium p-4 rounded-lg lg:w-80 lg:text-lg xl:w-96' type="text" placeholder='Course Description'/>
            <input {...register('price')} className='w-60 h-10 bg-[#01c8b5] outline-hidden text-black placeholder-black text-base font-medium p-4 rounded-lg lg:w-80 lg:text-lg xl:w-96' type="text" placeholder='Course Price'/>
